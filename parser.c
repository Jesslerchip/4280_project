...
ParseNode* vars(Token token, FILE* inputFile) {
    ...
    while (token.tokenID == KEYWORD_TK && strcmp(token.tokenInstance, "create") == 0) {
    ...
    // Check if no variable declarations were found
...
ParseNode* R(Token token, FILE* inputFile) {
    ...
    if (token.tokenID == OPERATOR_TK && strcmp(token.tokenInstance, "(") == 0) {
        ...
        addChild(node, createNode(token.tokenInstance));
        token = getToken(inputFile); // Consume the next token
...
ParseNode* stat(Token token, FILE* inputFile) {
    ...
    if (token.tokenID == KEYWORD_TK && strcmp(token.tokenInstance, "cin") == 0) {
        ...
        if (blockNode != NULL) {
            addChild(node, blockNode);
            addChild(node, createNode(";")); // Add semicolon after block
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (ifNode != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after if
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (loop1Node != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after loop1
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (loop2Node != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after loop2
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (assignNode != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after assign
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (labelNode != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after label
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (gotoNode != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after goto
        } else {
            freeParseTree(node);
            return NULL;
        }
        ...
        if (pickNode != NULL) {
            ...
            addChild(node, createNode(";")); // Add semicolon after pick
        } else {
            freeParseTree(node);
            return NULL;
        }
...
ParseNode* out(Token token, FILE* inputFile) {
    ...
    printf("DEBUG OUT: Token: %s, Token ID: %d, Line: %d, Char: %d\n", token.tokenInstance, token.tokenID, token.lineNumber, token.charNumber);
    token = getToken(inputFile);
    printf("DEBUG OUT: Token: %s, Token ID: %d, Line: %d, Char: %d\n", token.tokenInstance, token.tokenID, token.lineNumber, token.charNumber);
...
ParseNode* assign(Token token, FILE* inputFile) {
    ...
    if (token.tokenID == KEYWORD_TK && strcmp(token.tokenInstance, "set") == 0) {
        ...
        if (token.tokenID != ID_TK) {
            printf("PARSER ERROR: Expected identifier after 'set' in assignment at line %d, char %d\n", token.lineNumber, token.charNumber);
            freeParseTree(node);
            return NULL;
        }
        addChild(node, createNode(token.tokenInstance));
...
ParseNode* pickBody(FILE* inputFile) {
    ...
    if (firstStatNode == NULL) {
        printf("PARSER ERROR: Expected valid statement in pick body\n");
    ...
    if (strcmp(token.tokenInstance, ":") != 0) {
        printf("PARSER ERROR: Expected ':' after first statement in pick body at line %d, char %d\n", token.lineNumber, token.charNumber);
        freeParseTree(pickBodyNode);
        return NULL;
    }
    ...
    if (secondStatNode == NULL) {
        printf("PARSER ERROR: Expected valid statement after ':' in pick body\n");
        freeParseTree(pickBodyNode);
        return NULL;
    }
